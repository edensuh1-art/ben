...
public class TournamentView extends VBox {
    ...
    private void showDetailsDialog(Tournament tournament, ObservableList<Tournament> store, Window owner) {
        ...
        btnBracket.setOnAction(e -> {
            if (tournament != null && tournament.getCategory().equals("Volleyball")) {
                BracketView bracketView = new BracketView(tournament.getTeams());
            }
        }
        ...
        if (organizerMode) actionBox.getChildren().addAll(btnAssign, btnPlayers, btnDelete);
    ...
    private String formatTournament(Tournament t) {
        ...
        if (!t.getMatches().isEmpty()) {
            ...
            for (Match m : t.getMatches()) {
                ...
                String winner = m.getWinner() == null ? "In progress" : m.getWinner().getName();
