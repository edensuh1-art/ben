    ...
    public CreateTournamentView(ObservableList<Tournament> store, ObservableList<Team> teams, String username, String passward) {
        ...
        save.setOnAction(e -> {
            ...
            try {
                ...
                if (!tournament.getCategory().equals("Soccer") && doRobin()){
                    ...
                    for (Match match : matches){
                        for (Pool pool : pools) {
                            if (pool.getTeams().contains(match.getHome())) {
                                match.setID(pool.getID() + "-" + match.getID());
                                match.setAsRobin();
                            }
                        }
                    }
                ...
                boolean exists = store.stream()
                        .anyMatch(t -> t != null && t.getID().equals(tournament.getID()));

                if (!exists) {
                    store.add(tournament);
                } else {
                    for (int i = 0; i < store.size(); i++) {
                        Tournament t = store.get(i);
                        if (t != null && t.getID().equals(tournament.getID())) {
                            store.set(i, tournament);
                            break;
                        }
                    }
                }
                SaveLoadService.saveUser(username, passward, new ArrayList<>(store)); //causeing forever loop of file?
